...
class TaskManagementController extends GetxController {
  ...
  void postComment(String taskId,
      {bool isPrivate = false, String? assigneeId}) async {
    if (comment.text.isEmpty || view.value.id == '') return;
    ...
    try {
      taskRepository.saveComment(
          model, assigneeId ?? user.user.value.id, isPrivate);
  ...
  void submitWork(TaskModel task) async {
    try {
      ...
      final attachmentUrls = await taskRepository.uploadFiles(
          'Teams/${task.team}/Submission/', attachments);
      ...
      await taskRepository.saveSubmissionDetails(newSubmission, task.team);
      await taskRepository.updateTaskStatus(TaskStatus.inProgress, task);
